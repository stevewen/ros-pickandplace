...
int main(int argc, char **argv)
{
  ...
  if (client.call(srv))
  {
      ...
      for (int i = 0; i < num; i++)
      {
        ...
        res_y[1] = srv.response.center_y[i] + 0.1;
        res_z[1] = srv.response.center_z[i] + 0.1;
        ...
        res_y[5] = srv.response.center_y[i] + 0.06;
        res_z[5] = srv.response.center_z[i] + 0.06;
...
// STEP 5: PLACE THE OBJECT
  ...
  orient.setRPY(0.0 , M_PI/4 , M_PI/2);
