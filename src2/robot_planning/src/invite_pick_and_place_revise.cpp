...
// ****************************************************************
// load scenes using datas from depth camera 
...
// STEP 5: PLACE THE OBJECT
  ...
  drop_pose.position.x = 0.8;    // 1.0            //[meters]
  drop_pose.position.y = 0.0;    // 0.3            //[meters]
  drop_pose.position.z = 1.0;    // 0.9            //[meters]
  csda10f_move_group.setStartState(*csda10f_move_group.getCurrentState());
...
// STEP 5: PLACE THE OBJECT
  ...
  tf::Quaternion orient2;
  orient2.setRPY(M_PI , 0.0 , 0.0);  
  approach2_pose.orientation.x = orient2.x();
  approach2_pose.orientation.y = orient2.y();
  approach2_pose.orientation.z = orient2.z();
  approach2_pose.orientation.w = orient2.w();
  drop2_pose.position.x = -0.1;    // 1.0            //[meters]
  drop2_pose.position.y = 0.6;    // 0.3            //[meters]
  drop2_pose.position.z = 0.9;    // 0.9            //[meters]
  csda10f_move_group.setStartState(*csda10f_move_group.getCurrentState());
